...
import { LayoutDashboard, ShoppingCart, Briefcase, Settings, LogOut, User as UserIcon, Menu, X, Box, Play, ShieldCheck, Tag, Users, ChevronDown, Compass } from 'lucide-react'
...
const EXPLORE_ITEMS = [
  { href: '/modelos', label: 'Modelos STL', icon: Box },
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/90 backdrop-blur-sm">
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger render={
              <button className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                EXPLORE_ITEMS.some(i => pathname === i.href || pathname.startsWith(i.href + '/'))
                  ? 'bg-primary/10 text-primary'
                  : 'text-muted-foreground hover:text-foreground hover:bg-muted/60'
              }`}>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border/50 bg-background px-4 py-3 flex flex-col gap-1">
          {[...NAV_ITEMS, ...EXPLORE_ITEMS].map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
